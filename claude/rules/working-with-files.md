...
- ⚠️ **PyMuPDF（`import fitz`）が入っている前提のコードを書かない。** macOS の system python3 には通常入っておらず `ModuleNotFoundError` で落ちる。`pip install --user` も PEP 668（externally-managed-environment）で拒否されるため、その場での回復もできない
- 代わりに Homebrew の poppler / mupdf を使う。テキスト抽出は `pdftotext -layout`、座標付きの構造は `mutool draw -F stext`、画像化は `pdftoppm -png -r 150`
- **スキャン PDF（手書きの赤入れなど、テキストレイヤーが無いもの）** は `mutool draw -r 150 -o p%02d.png <file>.pdf` で全ページ PNG 化して画像読解する。判読しにくいページだけ `-r 300` で再レンダリングして拡大確認する
