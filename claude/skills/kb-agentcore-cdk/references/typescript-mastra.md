...
検証バージョン: `bedrock-agentcore@0.2.3`、Node.js 20 (Dockerfile) / v24 (ホスト)。`@aws/agentcore` は当時の最新を使っており固定していない
...
const agent = new Agent({
  ...
  model: bedrock(MODEL_ID),   // 実在は list-inference-profiles で確認したIDを入れる
});
...
| モデルIDのプレフィックス | Dockerfile の ENV AWS_REGION |
|-----------|------------------------------|
| `jp.` | `ap-northeast-1` |
| `us.` | `us-east-1` または `us-west-2` |
| `global.` | 任意（AWS 側がルーティングする） |
