...
**解決策**: アプリケーションのHTTPSクライアントからRuntimeエンドポイントを呼び、実行時に取得したJWTを `Authorization: Bearer {JWT}` ヘッダーへ設定する。

JWTをシェルのコマンドライン、履歴、URL、ログへ含めない。疎通確認でも固定トークンをファイルへ保存せず、有効期限の短いトークンを認証フローから取得する。
...
プレビュー段階のうちはバージョンを固定しない。仕様変更が速く、古い CLI は動かなくなるリスクがあるため常に最新を使う。
...
`aws-targets.json` には実際のアカウントIDが入るため、環境ごとのファイルは `.gitignore` に追加する。公開する場合は値をプレースホルダーへ置き換えたテンプレートだけを置く。
...
ただし、手順の動作確認が目的なら「**`aws-targets.json` 直書き＋`deploy -y --json`**」で完結する（最も安定）。
