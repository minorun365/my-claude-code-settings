...
典型的な症状は、5分間隔のような短い周期のジョブが `sha256(...)` などから組んだ固定セッションIDを使っていて、新しい Runtime バージョンを出しても旧プロンプト・旧モデルのまま動き続けることである。
