...
description: Strands Agents + Bedrock AgentCore のナレッジ。エージェント開発、ツール定義、CDK、Observability等（Identity認証は /kb-agentcore-identity）
user-invocable: true
---
...
→ `/kb-agentcore-identity` を参照（3LO/M2M/デコレータ分離/callback等）
...
→ `/kb-bidi-agent` を参照（Nova Sonic、WebSocket、カスタムI/O等）
