...
1回の依頼で同種のツールを何度も呼ばせる設計だと、モデルが**複数の tool use を1つの応答にまとめて生成**しようとして出力トークン上限に達し、`MaxTokensReachedException` で応答が途中で切れる。tool use の引数が長い（資料本文の要約を渡す等）ほど起きやすい。1件のダミー入力では偶然収まり、実データ規模で初めて踏むことがある。
