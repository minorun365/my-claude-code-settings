...
| スキル | ファイル | 内容 |
|--------|---------|------|
| `kb-strands-agentcore` | `SKILL.md` | Strands Agents フレームワーク（Agent作成、ツール定義、イベント処理） |
| `kb-agentcore-cdk` | `SKILL.md` | AgentCore CDK、デプロイ、ランタイム統合、コンテナ構成 |
| `kb-agentcore-observability` | `SKILL.md` | AgentCore Observability（OTEL、ログ、メトリクス、トレース） |
...
2. **該当するスキルファイルを特定**
   ...
   - Strands Agentsフレームワーク関連 → `kb-strands-agentcore`
   - AgentCore CDK/デプロイ/ランタイム関連 → `kb-agentcore-cdk`
   - AgentCore Observability関連 → `kb-agentcore-observability`
