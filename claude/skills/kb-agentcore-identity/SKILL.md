...
`handle_auth_url` は認証中の利用者へURLを一度だけ提示する処理として実装する。認可URLには一時的なstate等が含まれるため、アプリケーションログ、チャット履歴、監視サービスへ永続化しない。
...
async def handle_callback(session_id: str):
    # 実装ではstateと認証開始時の利用者・session_idの対応を検証する
...
3. エージェントを実行し、認証中の利用者だけにauth URLを提示してブラウザで認可
