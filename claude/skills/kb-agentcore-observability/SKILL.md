...
トレースにはプロンプト、LLMの応答、ツールの引数・結果、例外が含まれ得る。Observabilityを有効にする前に、次を決める。

- 個人情報、認証情報、顧客データを記録しないためのマスキング方針
- CloudWatch Logsの閲覧権限と保存期間
- 本番環境で本文を収集する必要性。不要なら本文を属性へ入れない
- 外部のObservabilityサービスへ送る場合のデータ分類、保存先、契約上の取扱い

認可URL、Bearerトークン、署名付きURL、Cookieはログやトレースへ記録しない。
