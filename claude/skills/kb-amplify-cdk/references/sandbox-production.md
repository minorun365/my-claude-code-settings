...
`npx ampx sandbox` は `.env` ファイルを自動読み込みしない。環境変数へ渡すのは、ログレベルや公開エンドポイントなどの非機密設定に限る。

```bash
LOG_LEVEL=DEBUG npx ampx sandbox
```

APIキーやパスワードはシェルで `.env` を一括展開せず、AmplifyのSecret managementや承認済みのシークレット管理サービスを使う。通常の環境変数はビルド成果物やCloudFormationイベントへ平文で現れる可能性がある。
...
- ブランチ名や公開エンドポイントなど、非機密のビルド設定を登録する
- APIキー等は通常の環境変数へ登録せず、AmplifyのSecret managementを使う
