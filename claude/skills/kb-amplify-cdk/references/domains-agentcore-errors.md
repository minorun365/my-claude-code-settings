...
**注意点**:
- 必要なアクション名はサービス更新で変わり得るため、AccessDenied の内容と最新の公式IAMリファレンスを照合する
- Runtime ARN 本体とエンドポイントのARNを個別に指定し、サービス全体のワイルドカード権限へ広げない
...
**原因**: aws-cdk-lib のあるバージョンから、型が `boolean` からオブジェクト型へ変更された。型エラーが出たら、まず手元の `aws-cdk-lib` のバージョンと当該プロパティの型定義を確認する
