...
検証ユーザーの固定パスワードやメール確認済みユーザーをInfrastructure as Codeで自動作成しない。テンプレート、CloudFormationイベント、カスタムリソースのプロパティへ認証情報が残る可能性があり、環境判定の不備で公開環境へ作成される危険もある。

- テストユーザーは対象環境を明示して必要な期間だけ作成する
- 初期パスワードはシークレット管理サービスなど、ソース管理外の経路で渡す
- 通知抑制や恒久パスワードを既定にせず、テスト終了後にユーザーを削除する
- CIでは専用のテスト用User Poolを使い、本番User Poolと分離する
