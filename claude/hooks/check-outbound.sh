...
#
# このフックは典型的な gh コマンドの誤操作を減らす補助策であり、セキュリティ境界ではない。
# 変数展開、alias、別CLI、APIクライアント等を完全には検出できないため、権限分離と事前確認を併用する。
#
# gh CLI の書き込み系コマンドを検出し、宛先リポジトリの owner を抽出して判定する。
# - 自分のリポジトリ（CLAUDE_OWN_GITHUB_OWNERS）への操作 → そのまま通過
...
# 自分のアカウントはローカル環境で設定する（スペース区切りで複数指定可）。
# 未設定時は安全側に倒し、すべての書き込み操作で確認する。
OWN_OWNERS="${CLAUDE_OWN_GITHUB_OWNERS:-}"
...
# 4. positional 引数の owner/repo（gh repo delete owner/repository 等）
